document.addEventListener('DOMContentLoaded', function() {
    function formValidation() {
        ...
        document.getElementById("contact_us_btn").addEventListener("click", function() {
            // Check if all form items are valid by triggering blur event on each field
            for (let i = 0; i < formFields.length; i++) {
                // Trigger blur event on each field to validate
                const event = new Event('blur');
                formFields[i].input.dispatchEvent(event);
            }
            
            // After validation, check if all fields are valid
            let allValid = true;
            for (let i = 0; i < formFields.length; i++) {
                if (!formFields[i].valid) {
                    allValid = false;
    ...
    // Show toast on page load if form was previously submitted
